...
RSpec.describe "on a merchant's discounts index page" do
  context "as a merchant employee" do
    ...
    it "can see a list of its discounts" do
      ...
      expect(page).to_not have_link(@discount_4.name)
    end
    it "can click a link to see specific discount" do
      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@merchant_user)
    ...
    it "can see a link to create a new discount" do
      allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(@merchant_user)
      visit "/merchant/discounts"
